(ns training.actor
  "TrainingActor — the ISCO-08 2424 community training actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one training operation request
  ...
  The unconditional invariant: the TrainingAdvisor can never
  directly commit a record the TrainingGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [training.advisor :as advisor]
            [training.governor :as governor]
            [training.store :as store]))

(defn build-graph
  "Build a compiled TrainingActor graph. `store` implements
  `training.store/Store`. `advisor` implements
  `training.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :module-ids (:module-ids proposal)
                                    :payload proposal}]
